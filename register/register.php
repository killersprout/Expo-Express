...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $query = "SELECT id FROM organizers WHERE username = ?";
    ...
    if(empty(trim($_POST["firstName"])) || empty(trim($_POST["lastName"])) ||
        empty(trim($_POST["organization"])) || empty(($_POST["event"])) || empty(($_POST["headcount"]))) {

        $firstNameErr = "Please enter first name.";
        $lastNameErr = "Please enter last name.";
        $organizationErr = "Please enter your organization.";
        $emailErr = "Please enter your email.";
        $eventErr = "Please tell us what event you want to host";
        $headCountErr = "Please tell us how many people you will have";
    }else{
    ...
    if(empty($userNameErr) && empty($passwordErr) && empty($confirmPassword_err) &&
        ...
        $query = "INSERT INTO organizers (username, password, firstname, lastname, organization, email, hash, event, headcount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if($stmt = mysqli_prepare($link, $query)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssi", $paramUserName, $paramPassword, $paramFirst, $paramLast, $paramOrganization, $paramEmail,$paramHash, $paramEvent, $paramHeadCount);
...
<div class="wrapper">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
        ...
        <!--Head count -->
        <div class="form-group <?php echo (!empty($headCountErr)) ? 'has-error' : ''; ?>">
            <label>Head Count</label>
            <input type="text" name="headcount" class="form-control" value="<?php echo $headCount; ?>">
            <span class="help-block"><?php echo $headCountErr; ?></span>
        </div>
        ...
        <?php if (count($_POST)>0) echo 'Standby while and administrator reviews your request.'; ?>
