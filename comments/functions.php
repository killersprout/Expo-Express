...
require_once "../nregister/functions.php";

// get post with id 1 from database Change this to sessions for diff people
$post_query_result = mysqli_query($link, "SELECT * FROM posts WHERE id=1");
$post = mysqli_fetch_assoc($post_query_result);

// Get all comments from database
$comments_query_result = mysqli_query($link, "SELECT * FROM comments WHERE postid=" . $post['id'] . " ORDER BY created_at DESC");
...
function getUsernameById($id){
    global $link;
    $result = mysqli_query($link, "SELECT username FROM exhibitors WHERE id=" . $id . " LIMIT 1");
    // return the username
...
function getRepliesByCommentId($id){
    global $link;
    $result = mysqli_query($link, "SELECT * FROM replies WHERE commentid=$id");
...
if (isset($_POST['comment_posted'])) {
    ...
    // insert comment into database//Using one because its hardcoded rn.
    $sql = "INSERT INTO comments (postid, userid, body, created_at, updated_at) VALUES (1, " . $userid . ", '$comment_text', now(), null)";
    $result = mysqli_query($link, $sql);

    // Query same comment from database to send back to be displayed
    $inserted_id = $link->insert_id;
    $res = mysqli_query($link, "SELECT * FROM comments WHERE id=$inserted_id");
...
if (isset($_POST['reply_posted'])) {
    ...
    $sql = "INSERT INTO replies (userid, commentid, body, created_at, updated_at) VALUES (" . $userid . ", $comment_id, '$reply_text', now(), null)";
    $result = mysqli_query($link, $sql);
    $inserted_id = $link->insert_id;
    $res = mysqli_query($link, "SELECT * FROM replies WHERE id=$inserted_id");
